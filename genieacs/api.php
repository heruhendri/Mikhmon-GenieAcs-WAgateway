...
function genieacs_api_request($endpoint, $method = 'GET', $data = null) {
    ...
    foreach ($urls_to_try as $url) {
        ...
        if (function_exists('curl_init')) {
            ...
            curl_setopt($ch, CURLOPT_TIMEOUT, 10); // Reduced timeout for faster failover
            ...
            $context_options = array(
                'http' => array(
                    'method' => $method,
                    'timeout' => 10, // Reduced timeout for faster failover
                    'ignore_errors' => true,
...
function genieacs_get_devices($query = null) {
    // Define parameters we want to fetch for devices
    $parameters = array(
        // Basic device info
    ...
    // First, try to get devices with projection to get specific parameters
    if ($query) {
        $query_param = $query;
    } else {
        $query_param = array();
    ...
    $endpoint = '/devices/?query=' . urlencode(json_encode($query_param)) . '&projection=' . urlencode($projection);
    ...
    // If there's an error with projection, try without projection
    if (isset($result['error'])) {
        // Try without projection
        if ($query) {
